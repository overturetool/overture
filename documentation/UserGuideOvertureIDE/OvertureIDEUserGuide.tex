...
module/class template to get you started by defining the
...
Eclipse templates can be particularly useful when modifying VDM models. If you
hit \textit{CTRL+space} after typing the first few characters of a template,
...
easier for users to construct models, even if they are not deeply familiar
with the VDM syntax.
...
It is possible to adjust and add to the templates defined in 
Overture. This can be done in the VDM preferences under
...
To execute or debug a VDM model, you must first create a debug launch
configuration\index{debug configuration}
from the menu \emph{Run} $\rightarrow $ \emph{Debug Configurations}\ldots. The
launch dialog requires the following information: the VDM project name,
...
configuration. Clicking one of the browse buttons will open a dialog which
gives you a list of choices - both the class/module and operation/function can be
chosen this way. If the chosen operation or function has parameters, argument
values to pass must be entered manually. You can also open the
...
line. When the debugger reaches the location of a breakpoint, you can
...
It shows the call stack of those that are suspended, and for VDM++ and VDM-RT stacks
...
This view shows all the variables in a thread context, allowing them to be
examined after a breakpoint has been reached. The variables and their values are
automatically updated when stepping through a model. The \emph{Variables view} is
located in the upper right hand corner in the \emph{Debug perspective}. It is
possible to inspect compound variables, expanding nested structures and soon.

\subsection{The Breakpoints View}

Breakpoints can be added in any perspective from the Editor view.
The debug perspective also has a \emph{Breakpoints view} that lists all current
breakpoints, allowing you to easily navigate to the location of a given breakpoint,
disable, delete or set the hit count or a break condition. The view is located in
the same panel as the \emph{Variables view} in the upper right hand corner.

\subsection{Conditional Breakpoints}
\label{sec:userguide:breakpoints}

Breakpoints can also be conditional. This is a powerful feature for the
...
mark in the Editor view and selecting \emph{Breakpoint
properties}\ldots. This opens a dialog like the one shown in
...
The \emph{Expressions view} allows you to write expressions that are evaluated
whenever the debugger stops.
Watched expressions can be added manually or created by selecting \emph{Create
watch expression} from the \emph{Variables view}. It is possible to edit existing
expressions. This view sits in the same panel as the \emph{Breakpoints view} and
the \emph{Variables view}.
...
given test suite\index{Test Coverage!Test suite} exercises the VDM model.
...
running a debug configuration, a new file with the \emph{.cov} extension will be created for 
each file in the project. These files are written into a project subfolder named
\texttt{generated/coverage/<date and time>}. Double clicking the .cov files
will open a special editor window that displays the source with coverage
coloured in red/green (red is not covered). Alternatively, a \emph{PDF} file
containing the entire model with coloured test coverage data can be generated 
by right clicking on the project name and choosing \emph{Latex} $\rightarrow$ 
\emph{Latex Coverage}.

\chapter{Pretty Printing to \LaTeX}\label{sec:prettyprint}

Include {\ttfamily overture.tex} which among other things makes use of the
{\ttfamily times.cls} and {\ttfamily listings.cls} style classes. This enables
the use of the standard {\ttfamily lstlisting} environment for type setting
source text and display it in a fixed width font where all VDM keywords
are typeset in bold.

It is possible to use literate programming/specification \cite{Johnson96} just as
inside VDMTools. Then you need to use the \LaTeX\ text processing system with
...
text-parts outside the specification blocks are ignored by the parser (but used
by the pretty-printer).

\chapter{Managing Proof Obligations}\label{sec:POmanagement}

In all VDM dialects, it is possible to identify places where run-time errors
\emph{could} potentially occur if the model was to be executed. The analysis of
these areas can be considered
as an additional check to the static type checking that is performed automatically.
...
creates so-called ``Proof Obligations''\index{proof obligation} for all the places
where run-time errors ``could'' occur. Each \emph{Proof Obligation} (PO)
is formulated as a predicate that must hold at a particular place in the VDM
model if it is error-free, and thus it may have particular context information associated with it.
POs can be considered as
...
selected VDM file. Right-click the project or file in the \emph{Explorer} view and
then select \emph{Proof Obligations} $\rightarrow$ \emph{Generate Proof
  Obligations}. Overture will then change into a special
...
\item A status field indicating whether the proof obligation is
  trivially correct or would have to be proved by a proof
  engine.
\end{itemize}
...
In order to automate part of the testing process, a notion of
...
special \emph{Combinatorial Testing} perspective that enables the automatic
unfolding of the traces and execution of each of the test
cases. Subsequently, the results of running the tests can be inspected
and erroneous test cases easily found. The user can then fix 
problems and re-run the traces definitions as development proceeds.

\section{Using the Combinatorial Testing GUI}

The syntax for trace definitions is defined in the VDM-10 language
manual.\index{traces}
If you have created a {\bf\ttfamily traces} entry for a module or class it
...
UML sequence diagrams can be used to indicate the test scenarios
...
is therefore used for detailed semantic analysis. The exchange between VDM and
...
embedded systems in VDM defines a step where one
moves from an initial VDM++ model to a VDM-RT model \cite{Larsen&09b}. This
step is supported by the Overture tool where it is possible to
copy a VDM++ project to create the starting point for a VDM-RT
project. This is done by right clicking on the VDM++ project to be
converted in the VDM Explorer view, followed by 
the \emph{Clone as VDM-RT} option.\index{create real time
  project}. A new VDM-RT project is then
created.\index{create!VDM-RT project} It will have the
same as the VDM++ project, but with \texttt{VDM\_RT} appended.
...
a {\bf\ttfamily system} class with appropriate declarations of
\texttt{CPU}s and \texttt{BUS}ses.
 
\chapter{Analysing and Displaying Logs from VDM-RT Executions}\label{sec:showlog}

When a VDM-RT model is executed, a logfile is created in
...
viewed with the build-in \emph{RealTime Log Viewer},\index{RealTime Log viewer}
...
The \emph{RealTime Log Viewer} also enables you to get an overview of
the model execution\index{model execution overview} at a system level -- this can be seen in
...
The \emph{RealTime Log Viewer} can
...
on the one CPU as well as the scheduling of concurrent processes. This can
...
to have existing legacy code that a user may not
wish to spend time modelling in VDM, but would like to make use of
from a VDM model. Overture has a feature enabling a combination of a VDM
model and external libraries with functionality in a standard \texttt{jar} file.
...
External libraries are coupled to VDM via
...
jar, calling out to a delegate java class. The delegate, if present, has
the same name as the VDM
module/class name where underscores (``\_'') are replaced with package naming
...
only when a \texttt{is not yet specified} body is first
reached in a class or module. The jar with the remote library must be places in
...
file needs to be the internal Values objects used in VDMJ. Documentation about these
can be found in the VDMJ Design Specification \cite{Battle09}.
...
public class sensor
{
	...
	{
		int result = ... // Read value for sensor number "id"
...
This is enabled in a fashion similar to the approach described in the 
previous section using an external java jar file. This feature corresponds
roughly to the CORBA based API from VDMTools \cite{APIMan}.
...
used, the Overture debugger continues working normally, so for example breakpoints can be
...
qualified name of the remote controller class must then be specified in the
launch configuration under the \textit{Remote Control} box.
...
\texttt{execute} on the Overture interpreter which is parsed to the remote
controller which then returns a string with the result. A more
advanced \texttt{valueExecute} is available which returns the internal
Value type of the interpreter which is useful for more complex
results. The
values that are to be exchanged between the Overture IDE and the jar
file need to be the internal Values used in VDMJ. Documentation about these
can be found in the VDMJ Design Specification \cite{Battle09}.
...
called VDMJ. This provides a command-line interface that may be valuable
...
So the first parameter indicates the VDM dialect to use and then
various extra options can be used. These are:

\begin{description}
\item[\texttt{-r}:] This will indicate the VDM release number (classic or vdm10).
...
\item[\texttt{runtrace <name> [test number]}:] This command runs the trace called
  ...
  that one test will be executed. \index{combinatorial testing}
...
\item[\texttt{filter \%age | <reduction type>}:] This command reduces
  ...
  ``\texttt{RANDOM}'', ``\texttt{SHAPES\_NOVARS}'', ``\texttt{SHAPES\_VARNAMES}''
   or ``\texttt{SHAPES\_VARVALUES}'' (note that lower case can also be used here).
\item[\texttt{assert <file>}:] This command runs assertions from the
  file provided. The assertions in the file must be Boolean
  expressions, one per line. The command evaluates each assertion in
...
\item[\texttt{pog}:] This command generates a list of all proof
  obligations for the VDM model that is loaded.\index{pog}\index{command!pog}  
...
\item[\texttt{break <function/operation> [<condition>]}:] This command
  creates a breakpoint at the start of a named function or operation and
  optionally makes it a conditional breakpoint. 
\item[\texttt{trace [<file>:]<line\#> [<exp>]}:] This command creates a
  tracepoint for a specific file and line. A
  tracepoint is similar to a breakpoint but with an implcit continue
  after it. A tracepoint prints the value of the expression given
...
\item[\texttt{coverage [clear|write <dir>|merge <dir>|<filenames>]}:]
  This command manages test coverage information. The coverage command displays the source
  code of the loaded VDM model (by default, all source files are
  listed), with ``+'' and ``-'' signs in the left hand column
  indicating lines which have been executed or not. Finally, the percentage
...
please report the occurrence to the SourceForge bug reporting utility
...
errors from the type checker. The errors here can be as
follows:
...
categories used in Overture, and an explanantion of the
circumstances under which each PO category can be expected.

\begin{description}
\item[map apply:] Whenever a map application is made it needs to be
...
\item[function apply:] Whenever a function application is used it needs to be
  certain that the list of arguments to the function satisfy the
...
\item[let be st existence:] Whenever a let-be-such-that
  expression/statement is used it needs to be certain that at least one value
...
\item[tuple selection:] This proof obligation category is used
  ...
  guranteed that the length of the tuple is at least as long as the
  selector used.
\item[value binding:] 
...
\item[while loop termination:] This kind of proof obligation is a
  reminder to ensure that a while loop is terminating.
\item[operation post condition:] Whenever an explicit operation has a
  ...
  of the operation satisfies the post-condition for all possible input.
