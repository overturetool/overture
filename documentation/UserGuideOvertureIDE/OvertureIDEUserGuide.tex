...
perspective\index{combinatorial testing}. 
...
different options can be used at the top level. These are:

\begin{description}
\item[\texttt{-w}:] This will suppress all warning messages.
\item[\texttt{-q}:] This will suppress all information messages.
\item[\texttt{-i}:] This will initiate the interpreter if the VDM
  model is successfully type checked, otherwise the errors discovered
  will be listed.
\item[\texttt{-p}:] This will generate all proof obligations for the
  VDM model provided if the model is synatx and type correct and then 
  stop the process.
\item[\texttt{-e <exp>}:] This will evaluate the \texttt{<exp>} print
  the result and stop the process.
...
\item[\texttt{-t <charset>}:] This will select a console character set.
\item[\texttt{-o <filename>}:] This will save the internal
  presentation inside
  \texttt{vdmj} after sucessful syntax and type checking. Such files are
...
\item[\texttt{-log}:] This will enable real-time event logging. These
...
the VDM model files only, giving any errors and warnings on
...
context of the specification loaded. One of the standard projects that
can be obtained as a part of the Overture distribution such as the
\texttt{shmem.vdmsl} example project. The interpreter could for
example be started as:
...
interactive interpreter commands are (those commands that have
abbreviated versions are provided in brackets afterwards): 
% (explanation below. The shmem source
%model is in Appendix A):

\begin{description}
\item[\texttt{modules}:] This command list the loaded module names in
  a VDM-SL setting. In case of a flat VDM-SL model the name
  \texttt{DEFAULT} is used. The default module will be indicated in
  the list displayed.\index{modules}\index{command!modules}
\item[\texttt{classes}:] This command list the loaded class names in
  both a VDM++ and a VDM-RT setting. The default class will be indicated in
  the list displayed.\index{classes}\index{command!classes}
\item[\texttt{default <module/class>}:] This command set the default
...
\item[\texttt{create <id> := <exp>}:] This command is only available
  for the VDM++ and VDM-RT dialects. It creates a named variable that
  can be used subsequently in the interpreter. It is mostly used for
  keeping track of global instances of
  classes.\index{create}\index{command!create} 
\item[\texttt{log [<file> | off]}:] This command can only be used in
  VDM-RT models and it will log events to the file indicated. By
  default, event logging is turned off. But logging can be enabled to
  the console by using log with no arguments, or to a file using log
  <filename>. Logging can subsequently be turned off again by using
  log off. The events logged include requests, activations and
  completions of all functions and operations as well as all creation
  of instances of classes, creation of CPUs and BUSses and deployment
  to specific CPUs and finally swapping in an out of
  threads.\index{log}\index{command!log}  
\item[\texttt{threads}:] This command can only be used for the VDM+p+
  and VDM-RT dialects and it lists the active threads with status
...
\item[\texttt{state}:] This command can only be used for the VDM-SL
  dialect and it shows the default
  module state.\index{state}\index{command!state}  
  The value of the state can be changed by operations called.
\item[\texttt{print <expression> (p)}:] This command evaluate the
  expression provided in the current
  context.\index{print}\index{command!print} Note that in case traces
  are defined in the VDM model using the syntax from
  Section~\ref{sec:syntax} then the \texttt{print} command can also be
  used for such a trace name. This will carry out the combinatorial
  testing expanding all the regular expressions and execute all of
  these. \index{combinatorial testing}   
\item[\texttt{assert <file>}:] This command run assertions from the
  file provided. The assertions in the file must all be Boolean
  expressions, one per line. Then this command execute each of them in
  turn, raising an error for any assertion which is
  false.\index{assert}\index{command!assert}  
\item[\texttt{init}:] This command re-initialize the global
  environment. Thus all state components will be initialised to their
  initial value again.\index{init}\index{command!init} 
\item[\texttt{env}:] This command list the value of all global symbols
  ...
  identifiers from value definitions and state definitions (in VDM++
  terminology instance variables). Note that it includes invariant,
...
\item[\texttt{pog}:] This command generate a list of all proof
  obligations for the VDM model that is
  included.\index{pog}\index{command!pog}  
\item[\texttt{break [<file>:]<line\#> [<condition>]}:] This command
  create a breakpoint at a specific file and line and optionally make
  it a conditional breakpoint.\index{break}\index{command!break}  
\item[\texttt{break <function/operation> [<condition>]}:] This command
  create a breakpoint in a function or an operation and optionally make
  it a conditional breakpoint. 
\item[\texttt{trace [<file>:]<line\#> [<exp>]}:] This command create a
  ...
  after it, so essentially this creates a trace of the values produced
  at a specific point in the console. \index{trace}\index{command!trace} 
\item[\texttt{trace <function/operation> [<exp>]}:] This command
  create a tracepoint. See \texttt{trace} above for an explanation of
  a trace point.
\item[\texttt{remove <breakpoint\#>}:] This command remove a
  trace/breakpoint referring to its number (can be seen by the
  \texttt{list} command). \index{remove}\index{command!remove} 
\item[\texttt{list}:] This command provide a list of all breakpoints
  in the order in which they have been
  created.\index{list}\index{command!list}  
\item[\texttt{coverage [<file>|clear]}:] This command display/clear
  ...
  percentage coverage of the source file is
  displayed.\index{coverage}\index{command!coverage}  
\item[\texttt{latex|latexdoc [<files>]}:] This command generate LaTeX
  line coverage files. These write \LaTeX\ files with parts of the
  ...
  output files are written to the same directory as source files, one
  per source file, with the extension \texttt{.tex}. Coverage
  information is reset when a specification is loaded, or when the
  command coverage clear is executed, otherwise coverage is
...
\item[\texttt{files}:] This command list all source files in the current VDM
  model. \index{files}\index{command!files} 
...
\item[\texttt{quit (q)}:] This command leave the
...
When the interpretation of a VDM model is stopped because of a
breakpoint there are additional commands that can be used. These are:

\begin{description}
\item[\texttt{step}:] This command step until the
  expression/statement on a new line.\index{step}\index{command!step} 
\item[\texttt{next (n)}:] This command step over function or operation
  calls and go to the next line with an
  expression/statement.\index{next}\index{command!next}  
\item[\texttt{out}:] This command run to the return of the current
  function or operation. \index{out}\index{command!out} 
\item[\texttt{continue (c)}:] This command resume execution and continues
...
\item[\texttt{up}:] This command moves the stack frame context up one
  frame.\index{up}\index{command!up}  
