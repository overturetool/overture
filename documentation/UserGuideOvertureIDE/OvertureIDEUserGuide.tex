...
interactive interpreter commands are: 
...
\item[\texttt{create <id> := <exp>}:] This command is only available
  ...
  keeping track of global instances of classes.
\item[\texttt{log [<file> | off]}:] This command can only be used in
  ...
  to specific CPUs and finally swapping in an out of threads.
\item[\texttt{state}:] This command can only be used for the VDM-SL
  dialect and it shows the default module
  state. The value of the state can be changed by operations called.
\item[\texttt{print <expression>}:] This command evaluate the
...
\item[\texttt{assert <file>}:] This command run assertions from the
  ...
  turn, raising an error for any assertion which is false. 
...
\item[\texttt{pog}:] This command generate a list of all proof
  obligations for the VDM model that is included.
...
\item[\texttt{trace [<file>:]<line\#> [<exp>]}:] This command create a
  tracepoint \fbox{PGL: what is a tracepoint?}
...
\item[\texttt{coverage [<file>|clear]}:] This command display/clear
  ...
  percentage coverage of the source file is displayed.
\item[\texttt{latex|latexdoc [<files>]}:] This command generate LaTeX
  ...
  \texttt{latex <file>}. 
\item[\texttt{files}:] This command list all files in the current VDM
  model. 
...
\item[\texttt{load <files>}:] This command replace current loaded VDM
  ...
  check of the files, the interpreter will exit after load.
\item[\texttt{quit}:] This command leave the interpreter.
\end{description}
...
\item[\texttt{step}:] This command step until the
  expression/statement on a new line.
\item[\texttt{next}:] This command step over function or operation
  calls and go to the next line with an expression/statement.
\item[\texttt{out}:] this command run to the return of the current
  function or operation. 
\item[\texttt{continue}:] this command resume execution and continues
...
\item[\texttt{up}:] This command moves the stack frame context up one frame.
\item[\texttt{down}:] This command moves the stack frame context down one frame.
\item[\texttt{source}:] This command lists VDM source around the
  current breakpoint. 
\item[\texttt{stop}:] This command terminate the execution immediately.
\end{description}
