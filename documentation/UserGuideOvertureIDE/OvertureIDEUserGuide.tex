...
\subsubsection{Adding additional Eclipse plugins}

Example here will be subclipse \fbox{KEL to write}
...
In this example a remote sensor will be defined which could read a
...
A VDM model can be remote controlled by the use of the java interface
\texttt{RemoteControl}. Remote control should be understood as a
delegation of control of an interpretation, this means that the remote
controller controls the debug session, it is responsible for taking
...
in a folder lib inside the project. And then specifying the full
qualified name of the remote controller in the launch configuration
...
In this example we have a VDM class A which just returns the input. As
seen in listing~\ref{remoteControllerJava} it is posible too call
...
Value type of the interpreter which is use full for more advanced
types. The
values that are to be exchanged between the Overture IDE and the jar
file needs to be the values used in VDMJ. Documentation about these
can be found in the VDMJ user manual \cite{Battle09}.
