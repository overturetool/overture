...
configuration\index{debug configuration}. Note that you need to create a debug
...
running this configuration, a new file with a \textt{.cov} extension will be created for 
each file in the project. These files are written into a project subfolder named
\texttt{generated/coverage/<date and time>}. Double-clicking the \textt{.cov} files
...
Include \textt{overture.tex} which among other things makes use of the
\texttt{times.cls} and \texttt{listings.cls} style classes. This enables
the use of the standard \textt{lstlisting} environment for type setting
