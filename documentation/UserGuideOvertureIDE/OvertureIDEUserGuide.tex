...
VDM models can be expressed in a specification language (VDM-SL) that
...
and built on top of the Eclipse platform. The project to develop the tools 
...
The \emph{Script Explorer view}\index{explorer} lets you create, select, 
...
to be given to the project and the simply select \texttt{Finish}. 
...
The \emph{Problems view}\index{problems} at the buttom of 
...
\emph{Close Project} entry in the 
...
\item[Invariant checks:] This is an option to the interpreter (default on) 
  for continuously also checking both state and type invariants of the values
  during interpretation of a VDM model. It is possible to swich off
...
different VDM source files shall be included. \fbox{TBW}
...
The expressions view allows the user to write expressions used for inspecting
values of the model. As for the variables view, the expressions are 
automatically updated when stepping throught the model.
...
only available for VDM-SL models if VDM-10 have been selected). 
...
and erroneus test cases can easily be found. The user can then fix 
the problem and reuse the same traces definitions.

\subsection{The Use of the Trace Definition Syntax}\label{sec:syntax}

The syntax for trace definitions are defined as:\index{traces}
...
object for which the operation (listed after the ``\texttt{.}'') is to
...
VDM models are not appropriate for describing everything and in
addition it is common to have existing legacy code that a user may not
wish to spend time on modelling use VDM but would like to make use of
from a VDM model. Therefore a feature enabling a combination of a VDM
model and a library with functionality in a standard \texttt{jar} file
has been enabled. Using this feature it is possible from a VDM model
to call functionality provided by the jar file. This functionality
corresponds to DL modules/classes in a VDMTools context \cite{DLMan}.

The external libraries which can be defined for VDM is coupled to the \texttt{is not yet specified} statements/ expressions. Only operations or functions of modules or classes can be delegated to an external jar.
The look up for a delegate java class is done based on the module/class name where \_ is replaced to dot: \texttt{remote\_lib\_sensor} becomes  \texttt{remote.lib.sensor} in java. The look up is only done once and only when a  \texttt{is not yet specified} statements/ expressions is reached.
The jar with the compiled remote library must be places in the project in a folder named \texttt{lib} it will then be put in the class-path of the interpreter when debugging. To compile a library jar for VDM the VDMJ jar needs to be in the class-path. It can be fund in the Overture IDE installation under \texttt{plugins/org.overture.ide.generated.vdmj\_2.0.1/lib}.

\subsection{Remote library example}
In this example a remote sensor will be defined which could read a value from a real sensor. The VDM model interface of the sensor can be seen in listing \ref{remoteSensorVdm} where as the Java class implementing it can be seen in listing \ref{remoteSensorJava}
...
front end (for example with a GUI) to interpreting different aspects
of a VDM model. This is enabled in a fashion similar to in the
previous section using a java jar file. This functionality corresponds
to the CORBA based API from VDMTools \cite{APIMan}.

A VDM model can be remote controlled by the use of the java interface \texttt{RemoteControl}. Remote control should be understood as a delegation of control of an interpretation, this means that the remote controller controls the debug session, it is responsible for taking action and executing parts of the VDM model when finished it should return and the debug session will stop. When a Remote controller is used the debugger continues working normally where breakpoints can be used.
Launching a debugging session with the use of a remote controller can be done by placing the jar with the remote controller in a folder lib inside the project. And then specifying the full qualified name of the remote controller in the launch configuration under the \textit{Remote Control} group.
 To compile a remote control jar for VDM the VDMJ jar needs to be in the class-path. It can be fund in the Overture IDE installation under\\ \texttt{plugins/org.overture.ide.generated.vdmj\_2.0.1/lib}.

\subsection{Example of a Remote Control class}
In this example we have a VDM class A which just returns the input. As seen in listing \ref{remoteControllerJava} it is posible too call \texttt{execute} on the interpreter which is parsed to the remote controller which then returns a string with the result. An more advanced \texttt{valueExecute} also exists which returns the internal Value type of the interpreter which is use full for more advanced types.
...
\newpage
\section{Adding external plug-ins to Overture IDE}
The Overture IDE is build as a RCP\footnote{Rich Client Platform} and there is no update manager included, however external plug-ins can still be manually installed.
To install a plug-in there are two things which needs to be done:
\begin{enumerate}
\item Copy the plug-in jar or jars into the plugin folder of the Overture IDE installation.
...
\end{enumerate}
After the steps above has been performed the IDE can be started and the new plug-ins will be available. (Remember to include all depended plug-ins which is not already a part of the Overture IDE).

\subsection{Adding SVN support to the Overture IDE}
Adding SVN support to the Overture IDE is rather simple:
\begin{enumerate}
\item Fetch the update site jars from \url{http://subclipse.tigris.org/} under Download and Install and select Zipped downloads. The latest version demonstrated in this guide is site-1.6.10.zip.
\item Copy the features and plugins folders into the Overture IDE folder, merging them with the ones of Overture. 
\item Then add the plugins to the \texttt{osgi.bundles} in the confi.ini file separated by comma.
\begin{itemize}
\item org.tigris.subversion.clientadapter
\item org.tigris.subversion.clientadapter\_1.6.10.jar
\item org.tigris.subversion.clientadapter.javahl
\item org.tigris.subversion.clientadapter.javahl\_1.6.9.3.jar
\item org.tigris.subversion.clientadapter.javahl.win32
\item org.tigris.subversion.clientadapter.javahl.win32\_1.6.9
\item org.tigris.subversion.subclipse.core
\item org.tigris.subversion.subclipse.core\_1.6.10.jar
\item org.tigris.subversion.subclipse.doc
\item org.tigris.subversion.subclipse.doc\_1.3.0.jar
\item org.tigris.subversion.subclipse.graph
\item org.tigris.subversion.subclipse.graph\_1.0.7.jar
\item org.tigris.subversion.subclipse.ui
\item org.tigris.subversion.subclipse.ui\_1.6.10.jar
\end{itemize}
When the steps above is completed the Overture IDE can be started and SVN support will then be available.
\end{enumerate}
