...
As described in Chapter~\ref{sec:linkToJava} integration between Overture and Java code can be established, either by writing native libraries in Java that can be called from VDM, or giving a Java program overall control of a VDM model, making calls to that model as a user interacts with a GUI.
...
This appendix describes the internal class hierarchy used by Overture to represent internal VDM model values, and describes how a Java program can convert these to native Java values (int, long, String etc.) as well as creating internal values for returning to the VDM model (e.g.\ as the return value of library methods).

\section{The Value Class Hierarchy}

All internal VDM values in Overture are held by instances of the \texttt{Value} class -– fully qualified name, \url{org.overturetool.vdmj.values.Value}. The Value class itself is abstract, but subclasses can be instantiated to represent any VDM value, such as a ``{\textbf\ttfamily seq of char}'' or a ``{\textbf\ttfamily nat1}'” or a value of an arbitrarily complex type. The hierarchy is shown in Figure~\ref{fig:gui:JavaVDMhierarchy}.

\begin{figure}[h]
\begin{center}
  \includegraphics[width=3.5in]{screenDumps/ValueHierarchy}
  \caption[labelInTOC]{Java Value Hierarchy}
  \label{fig:gui:JavaVDMhierarchy}
\end{center}
\end{figure}

Generally, the name of the Value subclass for a VDM type is \texttt{NameValue}, for example \texttt{BooleanValue} or \texttt{SeqValue}.
...
To convert a VDM Value into a Java value, the \texttt{Value} class provides a number of conversion methods, each of which either returns the corresponding Java primitive value, or throws an exception if the conversion cannot be made for the VDM type concerned:
...
Note that all of these conversion functions takes a Context parameter and can throw a ValueException. The context parameter is used internally by Overture and represents the call stack during the evaluation of an expression. This parameter can be set to null when using these methods in Java code outside Overture. The \texttt{ValueExceptions} are thrown if the VDM value cannot be converted into the Java type requested. For example, calling \texttt{booleanValue} on a \texttt{RealValue} object will raise a \texttt{ValueException} with the message text \texttt{"Cannot get bool value of real"}.

\section{Sets, Sequences and Maps}

VDM allows primitive types to be built into more complex aggregations and collections, and these can also be converted to and from Java types, though the process is a little more involved. Three classes are provided to help with this conversion: \texttt{ValueSet}, \texttt{ValueList} and \texttt{ValueMap} (all within the same  \url{org.overturetool.vdmj.values} package). These classes are, respectively, a Java \texttt{Set}, \texttt{List} and \texttt{Map} of VDM Values:
...
These three classes have obvious constructors, and allow Values (or Value pairs) to be added to the collection subsequently, using standard Java collection methods:
...
Using these three helper classes, it is now possible to create VDM set, sequence and map values, using constructors for the \texttt{SetValue}, \texttt{SeqValue} and \texttt{MapValue} classes:
...
Note that there is a special constructor for SeqValue that takes a Java string. This creates a VDM ``{\textbf\ttfamily seq of char}'', but without the need to create a \texttt{ValueList} with \texttt{CharacterValue}s.

If the \texttt{ValueList} (or other) collection passed to these constructors contains a mixture of VDM types -– i.e.\ a mixture of VDM Value subclasses, such as a BooleanValue and a NaturalOneValue -– then the type of the constructed VDM value is the union of the various types passed, in this example ``{\textbf\ttfamily seq of (bool | nat1)}''. If this VDM type is not compatible with the use of the value in the rest of the VDM model, runtime type exceptions may occur when the value is processed by the model.
...
Note that, as with the \texttt{SeqValue} constructor, there is a special method to return a Java String from a ``{\textbf\ttfamily seq of char}'' \texttt{SeqValue}, rather than a \texttt{ValueList} of \texttt{CharacterValue}s. As before, if the \texttt{Value} being used is not a sequence or set or map, then these methods will throw a \texttt{ValueException}.

\section{Other Types}

The sections above describe enough to create or deconstruct simple VDM values in Java, and simple collections of those values (collections of collections etc.). The remainder of this section describes the remaining unusual cases, for more sophisticated types.

\section{Function values}

Overture has an internal \texttt{FunctionValue} class, and this is used to hold the values of functions (e.g.\ the value of a ``{\textbf\ttfamily lambda}'' expression or the value of a function defined within a module). But as far as Java is concerned, these values are opaque -– there is no equivalent Java construct, and the only way to evaluate a VDM function is to let Overture perform that evaluation. Similarly, Java cannot construct a \texttt{FunctionValue}.

The only operation that Java can reasonably perform with a \texttt{FunctionValue} is to create a composite function (eg. “f1 comp f2” in VDM) or a function iteration (e.g.\ ``\texttt{f ** 3}'' in VDM) using existing \texttt{FunctionValue}s. To do this, there are two subclasses of \texttt{FunctionValue}, called \texttt{CompFunctionValue} and \texttt{IterFunctionValue}, the constructors for which are as follows:
...
There is a method to obtain a \texttt{FunctionValue} from a \texttt{Value}, but note that this is not an internal Java value (unlike other methods on Value, like \texttt{realValue}). It is used as a more convenient method than casting the Value to a \texttt{FunctionValue}, but that is all it does.
...
When VDM++ and VDM-RT create new objects using the ``{\textbf\ttfamily new}'' operator, the resulting values are held as \texttt{ObjectValue}s in Overture. These are complex types that involve function and operation definitions for the object as well as any type, value, sync, thread or traces sections defined. Therefore, like \texttt{FunctionValue}s, \texttt{ObjectValue}s are really opaque to Java and cannot be used directly.

Like \texttt{FunctionValue}s, \texttt{ObjectValue}s have a method to convert a Value into an \texttt{ObjectValue}. But this only performs a cast:
...
A VDM record is just a collection of typed field values. A \texttt{RecordValue} can be obtained from a \texttt{Value} using the following method, which like \texttt{FunctionValue} and \texttt{ObjectValue}, just returns a \texttt{RecordValue} rather than some other Java representation:

\begin{lstlisting}[language=JAVA]
public RecordValue recordValue(Context ctxt)
\end{lstlisting}

To get individual field Values from a \texttt{RecordValue}, two more Java helper types have to be introduced, called FieldMap and FieldValue. A FieldValue has the following constructor, and represents on field in a record:

\begin{lstlisting}[language=JAVA]
public FieldValue(String name, Value value, boolean comparable)
\end{lstlisting}

The comparable argument indicates whether this field is used in the value comparison between record values. A field declared with ``\texttt{:-}'' in VDM would have a false argument, but normally this would be true, and the value must match the record type being used. \texttt{FieldValue}s are added to a \texttt{FieldMap}, which is just a Java List of \texttt{FieldValue}s.

So given a \texttt{RecordValue}, its \texttt{FieldMap} can be obtained from a public final field in the object, called \texttt{fieldMap}\footnote{Really this ought to have a get method.}, and from there, individual \texttt{FieldValue}s obtained -– e.g.\ \texttt{fieldMap.get(0).name} and \texttt{fieldmap.get(0).value}.

To create a \texttt{RecordValue}, the record type is obtained from the \texttt{RemoteInterpreter}:

\begin{lstlisting}[language=JAVA]
type = remoteInterpreter.getInterpreter().findType(“Name”)
\end{lstlisting}
...
The caller is responsible for passing field Values that matches their expected type. If they do not match, Overture will throw a dynamic type exception during subsequent evaluations.

\section{Token Values}

Token values are really just wrappers around normal VDM Values, reflecting the way they are created in VDM, like \texttt{mk\_token("hello")}, which would be a wrapper to a ``{\textbf\ttfamily seq of char}''. There is no special way to get a TokenValue from a Value, other than to cast it. Having cast the value, the wrapped value can be obtained from the public final Value field called ``\texttt{value}''.
...
A \texttt{TupleValue} in Overture is just a wrapper for a \texttt{ValueList}. The following method and constructor are used as expected:
...
A VDM type can be given a name and an invariant, wrapping an otherwise more primitive type without an invariant. Overture has a separate Value subclass for values of such types that simply put the primitive Value together with a FunctionValue for the invariant. However, as with RecordValues (which can also have invariants), it is not currently possible to create InvariantValues from Java for types that have an invariant.
...
The \texttt{NamedType} is obtained in a similar way to the \texttt{RecordType} above, using the \texttt{Remote\-Interpreter}. Note that the caller is responsible for passing a \texttt{Value} that matches the expected type. If they do not match, Overture will throw a dynamic type exception during subsequent evaluations.
