...
The Overture initiative is a platform that enables researchers, students and practitioners to experiment with software modelling languages and tools~\cite{LarsenBFL08}\fixme[margin]{Update this bib entry because there are now more authors\ldots}.
...
The APS was designed by Sander Verm\"olen during his MSc project~\cite{SanderThesis}\fixme[margin]{Search for Sander's thesis reference at home.}.
...
The contribution described in this paper focused on the development of the core of the APS and fringe interfaces for external components, allowing for a loose coupling between the core and the other components.
...
However, from the experience gathered from using the APS in some case studies~\cite{FerreiraSO2008,Ferreira2009,SBMF09paper}\fixme[margin]{Add bib entry for SBMF09 paper.}, the tactics selection as done by the translation tool is suboptimal as the same tactic gets chosen for each and every type of PO.
...
There are also problems in the translation that were already identified by Verm\"olen in his thesis~\cite[OpenIssues]{SandersThesis}\fixme[margin]{Add reference to Sander's thesis open issues, or to the website.} but remain unresolved.
