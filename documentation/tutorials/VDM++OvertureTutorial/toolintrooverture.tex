...
debugging using Overture. Section~\ref{sec:CT} shows how
parts of the test process can be automated using Overture's
combinatorial testing feature.  Section~\ref{sec:PO} demonstrates how
it is possible automatically to generate the additional
checks (called \emph{proof obligations}) needed ensure that a model is
...
to export the UML model from EA to XMI format. This
is then imported subsequently into Overture~(see Figure~\ref{fig:xmiexport}).
...
Explorer view by double clicking.  A new VDM++ project is created by choosing
...
the generated VDM files. When making corrections to the model, you can
...
will propose the use of an implicit or explicit operation template.
...
in VDM. Breakpoints can easily be set by double clicking in left
...
The Debug perspective is illustrated in
...
When any evaluation is performed on a VDM model, VDMJ records the lines of the
VDM model that are executed. This permits the line coverage to be examined
after a test to identify the parts of the VDM model that have not yet been
exercised -- coverage is cumulative, so a set of tests can be executed and their
total coverage examined at the end.


In our simple example, the single test does cause the majority of the
VDM model to be executed, but the \verb|NumberOfExperts| operation in
\verb|plant.vdmpp| remains unused. To display coverage, the 
\verb|coverage <file>| command is used. 
Lines that have been executed are listed with a '+'
prefix, while those that have not have a '-' prefix; lines that are not
executable have no prefix. Alternatively, the coverage can be written
to a \LaTeX\ file using the \verb|latex| command:

\begin{lstlisting}
> coverage plant.vdmpp
Test coverage for plant.vdmpp:
  ...
...
> latex AlarmPP/Plant.vdmpp
Latex coverage written to AlarmPP\Plant.vdmpp.tex
> 
\end{lstlisting}
