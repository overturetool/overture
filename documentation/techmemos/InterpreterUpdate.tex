...
\section{Main issues that needs adressing}

The current version of the Overture/VDM interpreter have a number of
issues that definitely needs adressing to make it useful for modelling
...
\item Whenever a multi-threaded VDM model is present the interpreter
  make use of multithreaded java threads and this result in a
...
\item Whenever the interpretation is stopped with a breakpoint in the
  interpretation of a multi-threaded VDM model only the thread that is
  broken is stopped (or threads that are created in a break
  state). This means that other threads continue their execution. This
  is not the way this should work because this both means that these
...
\item The permission predicates are only reevaluated periodically (in
  ...
  that windows of oppertunities where the permission predicates can
...
\section{Main issues that would be nice to adress}

In addition to the issues that needs to be solved there is a number of
issues that have been discussed at different points of time and are
...
\item Handling of static instance variables in instances of classes
  that are deployed to multiple cpus are not handled appropriately
  ...
  real world. This may require a language chnage so this needs to be
  clarified and a proposal will have to be made for the LB when this
  is done.
\item At the moment all deployment of instances of classes happen
  ...
  dynamic allocation (for example inspired from teh CREDO project) in
  a VDM-RT context. This again will probably require language chnages
...
\item At the moment both Overture/VDM and VDMTools have a single
  ...
  be desibale to explore the possibility of defining multiple virtual
  cpus for the system class.
\item Incorporating of predicates over the events happening over time
  ...
  graphically in visualisations of logfiles).
\end{enumerate}

\section{Suggested Solutions}

It is sugegsted to keep the use of java threads in the Overture/VDM
interpreter but have a master thread that controls all threads. This
...
just be one thread executing and thus the nondeterminism arasing from
...
reevaluation of permission predicates extra stuff will need to be
...
When we talk about the VDM-RT interpreter the master thread will need
to carry out a round-robin investigation of what functionality each
resource (cpu and bus) can execute before updating the time and once
all have been processed the one with the smallest time step will be
chosen and all threads will be moved forward with this amount of
time. For the treads that have a longer duratiojn this means that the
remaining duration will be decremented accordingly. For those that
have dirations equal to the minimum duration the transactions
performed in that local thread will be committed and made available to
all othert threads. This is also the place where the co-simulation
will be incorporated (before the cpus are asked to progress time with
the smallest time step the co-simulation will be asked to progress
this amount of time and either that will happen and values will be
exchanged or an event happen at a smaller amount of time and then all
cpus are asked to progress with that amount). 
