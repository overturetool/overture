...
%The \texttt{TimeStamp} class is a subclass of 
...
sleep. As long as the {\bf\ttfamily{threadid}} is in the
...
%\textbf{tid} of the thread that it is supposed to be notified each clock
%tick.
%
%\begin{lstlisting}
...
Note here that the {\bf\ttfamily{duration}}\texttt{(0) }{\bf\ttfamily{startlist}}\texttt{(...)} statement is
...
to as clients. 
%The main class in the Time Stamp pattern is the \texttt{TimeStamp} class, which extends the \texttt{WaitNotify} class described in Section~\ref{sec:waitnotify}. 
...
time steps. Each client executes its
...
is then called in order to release any waiting clients. Finally, \texttt{TimeStamp`Awake}
is called, where the thread waits until it is time to wake up. Note that 
since the clients are incidental to the pattern, they are not given in the following specification.
