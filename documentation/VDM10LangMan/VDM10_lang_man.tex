...
\item[Semantics:] A {\it Set enumeration} has the form:
  ...
  cannot be executed by the VDM interpreters.
...
\item[Semantics:] A {\it bind} matches a pattern to a value. In a {\it
    ...
    executed by the VDM interpreters.  This would require the VDM interpreters
...
loop the VDM interpreters will increment its internal clock by 10 time units,
...
\texttt{`}-sign (back-quote) (see also chapter~\ref{sec:nameconflicts}).
...
\Rule{trace conc}{
  \Lit{||}, \Lit{(}, \Ruleref{trace definition list}, 
  \SeqPt{\Lit{,}, \Ruleref{trace definition list}}, \Lit{)}
}
...
The \emph{trace conc} definitions are similar to the
...
VDM++ models where potential deadlocks can occur under some circumstances.
