...
revision. These dialects are supported by both VDMTools (in the
appropriate version) as well as in the Overture open source tool built
on top of the Eclipse platform. Whenever a construct is common to the
three different dialects the term ``VDM languages'' will be
used. Whenever a construct is specific to a subset of the VDM
languages the specific dialect term mentioned above will be mentioned
explictly. 
...
Real-Time embedded and distributed systems~\cite{Verhoef&06b}.
...
functions can be polymorphic. In VDM++ and VDM-RT access modifiers
including {\bf\ttfamily static} can
be used. In general, the syntax for the definition of a
function is:
...
Details of the access and \keyw{static} (can only be used in VDM++ and
VDM-RT) specifiers can be found in
...
\item[Semantics:] The {\it threadid expression} has the form:
  ...
  periodic threads gets a new threadid at the start of each period.
...
Details of the VDM++ and VDM-RT access and \keyw{static} specifiers
...
\Ruledef{escape sequence}{
  ...
  \Lit{{\char'134}x} \hyperlink{charSetTable}{hexadecimal
    digit},\hyperlink{charSetTable}{hexadecimal digit}, \hyperlink{charSetTable}{hexadecimal digit},\hyperlink{charSetTable}{hexadecimal digit} \dsepl
  \Lit{{\char'134}u} \hyperlink{charSetTable}{hexadecimal digit},\hyperlink{charSetTable}{hexadecimal digit} \dsepl
