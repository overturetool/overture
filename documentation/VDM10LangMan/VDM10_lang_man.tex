...
away and aborted). Thus if one would like to ensure a specific number
of such other threads to be completed before stopping the execution
ons needs to block the debug thread using a synchonisation as
...
\Ruledef{periodic obligation}{
  \Lop{periodic},\Lit{(}, \Ruleref{numeral}, \Lit{)}, \Lit{(}, \Ruleref{name}, \Lit{)}
}
