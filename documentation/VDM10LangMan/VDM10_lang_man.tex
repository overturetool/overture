...
\item[Semantics:] A {\it bind} matches a pattern to a value. In a {\it
    ...
    to the same set or type.  Notice that type binds {\bf can} only be
...
\item[Semantics:] The {\it assignment statement} corresponds to a
  ...
  checking, thread switching or extra duration. It is as if the
...
\item in contrast to \texttt{start} and \texttt{startlist}, \texttt{stop} and \texttt{stoplist} will block until the thread(s), identified by their object identifiers that are passed as an argument, are dead.
\item passing the thread identifier of a task that is already dead, will yield a run-time error
\item killed threads can be restarted by calling \texttt{start} and \texttt{startlist} again
...
\section{Sporadic Thread Definitions}\label{se:sporadic-thread}
...
The type of the expressions must yield natural number, as we use a natural number valued wall-clock in the interpreter, otherwise a run-time error will occur. Note that the evaluation of the expressions causes time to elapse whenever the \Lop{start} or \Lop{start\_list} statement is executed. The expressions all denote a time value with a resolution of 1~nsec.

\item[Semantics:] The type of the expressions should all yield a natural number (as we use a natural number valued wall-clock in the VDM-RT interpreter), otherwise a run-time error will occur. Note that the evaluation of the expressions also causes time to elapse whenever the \Lop{start} or \Lop{start\_list} statement is executed. The expressions all denote a time value with a resolution of 1~nsec.
...
this simply result in sequencing between the \emph{trace definition
  terms} used inside it.

In the \emph{trace definition term} it is possible to introduce
alternatives using the bar (``\texttt{|}'') operator. This result in
