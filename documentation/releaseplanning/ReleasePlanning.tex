...
\item \markFuture{ Documentation of the Overture IDE in order to enable more
...
\item \markFuture{Provide help (F1) inside Eclipse}, \markDone{as a first go we just add pdf files to the welcome page}; \developer{AugustoRibeiro}

\item \markFuture{ provide goto definition and completion; \developer{AugustoRibeiro}}

\item \markFuture{Complete the new ASTGen utility and add all sources for it to
  the tools utility in the Overture SF SVN. \developer{NickBattle} \textbf{Dependencies: None}}

\item \markFuture{Change to the AST generated from the new version of ASTGen all
...
\item[Release 1.0.0 (February 2011):] The aim for this release was to
  include the following features:   
\begin{enumerate}
\item \markFuture{Quick fix incorporated in the editor view of the IDE for all VDM
  dialects. \developer{??}}
\item \markFuture{Goto definition in the edit view of the IDE across all files in
  a project for all VDM dialects. \developer{??}}
\item \markFuture{First version of refactoring support for all VDM
  dialects. \developer{??}}
...
\item \markFuture{ Include VDMDoc (inspired by JavaDoc) for all VDM dialects. \developer{??}}
\item \markFuture{Stable version of the UML mapper enabling proper round-trip
...
\item \markFuture{First release of code generator from VDM to a programming
  language. \developer{MarcelVerhoef}}
\item \markFuture{Increased \LaTeX\ support in the editor such that parts outside
