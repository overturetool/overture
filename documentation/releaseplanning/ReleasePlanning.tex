...
% VDM chunks by starting with a `?Å¥ character and ending with a `Å£Å¥
%\lstset{escapeinside=?Å£}
...
\begin{enumerate}
\item Analyse whether we can go for a debugger without
...
\item \markDone{ The combinatorial testing feature will be updated in the IDE
  ...
  dialects. \developer{KennethLausdahl}, \textit{Done(Prototype): 2. feb 2010 - Kenneth}}
...
\item \markDone{Syntax and} type check the arguments used in the debug
  configuration. \developer{KennethLausdahl}, \textbf{Dependencies: None}
...
\item Documentation of the Overture IDE in order to enable more
  ...
  and the AST access. \developer{AugustoRibeiro}, \textbf{Dependencies: \ref{completeAST}, \ref{analyseDLTKDebug}}
...
\item Create a new explorer  for VDM in the same style as ``script explorer'' named ``VDM
  explorer''. \developer{AugustoRibeiro}, \textbf{Dependencies: \ref{picturesEclipse}} 
...
\item[Release 0.3.0 (June 2010):] The aim for this release is to
  include the following features:  
\begin{enumerate}
\item Launch configuration setup: 
	\begin{enumerate}
  ...
  the name of the project (and not just ``New configuration'').
	...
	\end{enumerate} \developer{AugustoRibeiro}, \textbf{Dependencies: R020:\ref{analyseDLTKDebug}}\label{launchconfig}
...
\item Making sure that the run and debug buttons in the IDE work
  ...
  supported). \developer{AugustoRibeiro}, \textbf{Dependencies: \ref{launchconfig}} 
...
\item A button for moving a test case from the combinatorial testing
  ...
  selected. \developer{KennethLausdahl,ChristianThillerman}, \textbf{Dependencies: \ref{launchconfig}} 

\item Stop all threads when a breakpoint or a run-time error has
  occured in one of the threads. \developer{NickBattle,++}, \textbf{Dependencies: R020:\ref{toplevelscheduling}} 

\item In the debug perspective it shall be possible to look into the
  contents of objects in the variables view. \developer{??}, \textbf{Dependencies: R020:\ref{analyseDLTKDebug},\ref{completeAST}} 

\item Include all definitions in the Outline view for all VDM
  dialects. \developer{AugustoRibeiro}, \textbf{Dependencies: R020:\ref{completeAST}} 

\item When creating new projects (by wizard) in the IDE include the possibility
...
\item Make sure that proof obligations for standard libraries are
  ignorred. \developer{NickBattle}, \textbf{Dependencies: None}  

\item Make it posible for users to provide static implementations to ``is not yet specified'' functions and
...
\item Make it possible for users to add a java implementation of a VDM class by adding a jar to the lib folder. The implementation should be done so one instance in java corresponds to one instance in VDM. No static. \developer{KennethLausdahl,NickBattle}, \textbf{Dependencies: \ref{staticIsNotYetSpecified}} 
\end{enumerate}
...
\item VDM Tools
	\begin{enumerate}
		...
		\item Create a VDM Tools project file from a Overture project and launch the VDM Tools GUI.
...
\item Coverage Editor

\item DLTK replacement
...
\item Develop development environment for DESTECS enabling different
  plug-ins to be added and docuenting this. \developer{KennethLausdahl}
