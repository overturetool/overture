...
\item Change to the AST generated from the new version of ASTGen all
  ...
  UML, JML and POtrans components). \fbox{NickBattle,KennethLausdahl}
...
\item In the edit perspective automatic type checking will only be
  ...
  errors. \fbox{KennethLausdahl}, \textit{Done: 1. feb 2010 - Kenneth} 
\item When the Overture tool is first started up and the user kills
  ...
  automatically. \fbox{AugustoRibeiro}, \textit{Done: 1. feb 2010 - Kenneth}
\item Syntax and type check the arguments used in the debug
  configuration. \fbox{KennethLausdahl}
\item Remove all communication exceptions from VDMJ which causes the
  ...
  broken). \fbox{NickBattle,KennethLausdahl}
...
\item Specify the top level scheduling strategy for the interpreter
  ...
  \hfill \fbox{KennethLausdahl,++} 
\item Make the pretty printer with test coverage usable for all VDM
  dialects. \fbox{KennethLausdahl,NickBattle}
\item Changing ``script explorer'' to ``VDM
  explorer''. \fbox{AugustoRibeiro} 
\end{enumerate}
...
\item When creating a new debug configuration it shall be named after
  ...
  configuration''). \fbox{AugustoRibeiro} 
...
\item A button for moving a test case from the combinatorial testing
  ...
  selected. \fbox{KennethLausdahl,ChristianThillerman} 
...
\item Make sure that proof obligations for standard libraries are
  ignorred. \fbox{NickBattle} 
\item Add static DL classes and modules to Overture where the user
  shall place corresponding jar files in the lib directory of the
  project implementing ``is not yet specified'' functions and
  operations . \fbox{KennethLaudahl,NickBattle}
\item Investigate whether dynamic an external jar files can be
  incorporated also. \fbox{KennethLausdahl,NickBattle}
\end{enumerate}
...
\item New version of the interpreter for scheduling of multiple
  ...
  break after a fixed amount of time. \fbox{KennethLausdahl,++}
...
\item Develop development environment for DESTECS enabling different
  plug-ins to be added and docuenting this. \fbox{KennethLausdahl}
