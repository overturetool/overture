...
public class ExtensionGenerator
{
	...
	public static void setExtendName(Environment baseEnv,String extendName)
	{
		...
		for (IInterfaceDefinition def : baseEnv.getAllDefinitions())
		{
			...
			// || (extendMode == ExtendMode.Extend && !baseClasses.contains(def)))
			{
	...
	public static Environment extendWith(Environment baseEnv,ExtendMode mode, Environment env)
	{
		switch (mode)
		{
			case Extend:
	...
	private static Environment changeExtendsTo(Environment baseEnv,Environment env)
	{
	...
	public static Environment extendWithStandalone(Environment baseEnv,Environment env)
	{
			...
			if (baseEnv.isTreeNode(c))
			{
				switch (baseEnv.classToType.get(c))
				{
					...
					case SubProduction:
						break;
					case Token:
						c.getName().setPackageName(baseEnv.getDefaultPackage()+ ".tokens");
						break;
					case Unknown:
						break;
	...
	private static void extendWith(Environment baseEnv,Environment env, IClassDefinition c,
			IClassDefinition cNew)
	{
