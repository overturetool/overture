package org.overturetool.eclipse.plugins.editor.core;

public class OvertureConstants {
	public final static String OVERTURE_PARTITIONING = "__overture_partitioning";
	
	public static final String LAUNCHING_PLUGIN_ID = "org.overturetool.eclipse.plugins.launching";
	
	public static final String VDMJ_INTERPRETER_ID = LAUNCHING_PLUGIN_ID + ".internal.launching.VDMJInstallType";
	
	public static final String VDMTOOLS_INTERPRETER_ID = LAUNCHING_PLUGIN_ID + ".internal.launching.VDMToolsInstallType";
	

}
