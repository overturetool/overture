...
import org.overturetool.eclipse.plugins.editor.internal.ui.formatting.DefaultCodeFormatterConstants;
import org.overturetool.eclipse.plugins.editor.scriptdoc.OvertureHeuristicScanner;
import org.overturetool.eclipse.plugins.editor.scriptdoc.OvertureIndenter;
 ...
 * 
 * @see org.eclipse.OvertureHeuristicScanner.internal.ui.text.JavaHeuristicScanner
 * @see org.eclipse.OvertureIndenter.internal.ui.text.JavaIndenter
 * @since 3.0
 */
public class IndentAction extends TextEditorAction {
	...
	public void run() {
		...
		if (document != null) {
			...
			try {
				...
				//DLTKCore.log(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "", e)); //$NON-NLS-1$
				return;
			}
			...
			Runnable runnable = new Runnable() {
				public void run() {
					...
					try {
						OvertureHeuristicScanner scanner = new OvertureHeuristicScanner(
								document);
						OvertureIndenter indenter = new OvertureIndenter(document,
								scanner, getJavaProject());
	...
	private boolean indentLine(IDocument document, int line, int caret,
			OvertureIndenter indenter, OvertureHeuristicScanner scanner,
		...
		if (offset < document.getLength()) {
			...
			ITypedRegion startingPartition = TextUtilities.getPartition(
					document, IOverturePartitions.OVERTURE_PARTITIONING, offset,
					false);
			String type = partition.getType();
			if (type.equals(IOverturePartitions.OVERTURE_DOC)
					|| type.equals(IOverturePartitions.OVERTURE_COMMENT)) {
				...
				// slashes+= 2;
				// dont indent inside the //. its ugly and it screws up the
				// indenter
		...
		int end = scanner.findNonWhitespaceForwardInAnyPartition(wsStart,
				offset + lineLength);
		if (end == OvertureHeuristicScanner.NOT_FOUND) {
			// an empty line
	 ...
	 * @since 3.1
	 */
	private String computeJavadocIndent(IDocument document, int line,
			OvertureHeuristicScanner scanner, ITypedRegion partition)
			throws BadLocationException {
		...
		int nonWS = scanner.findNonWhitespaceForwardInAnyPartition(lineStart,
				lineEnd);
		if (nonWS == OvertureHeuristicScanner.NOT_FOUND
				|| document.getChar(nonWS) != '*') {
			if (nonWS == OvertureHeuristicScanner.NOT_FOUND)
		...
		if (previousLineNonWS == OvertureHeuristicScanner.NOT_FOUND
				|| document.getChar(previousLineNonWS) != '*') {
			// align with the comment start if the previous line is not an
			// asterisked line
			previousLine = document.getLineInformationOfOffset(partition
					.getOffset());
		...
		String indentation = document.get(previousLineStart, previousLineNonWS
				- previousLineStart);
