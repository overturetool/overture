...
import org.overturetool.eclipse.plugins.editor.internal.ui.text.OverturePreferenceInterpreter;
...
public final class OvertureIndenter {
	...
	private final class CorePrefs {
		...
		OverturePreferenceInterpreter prefs = new OverturePreferenceInterpreter(
	 ...
	 */
	private final OvertureHeuristicScanner fScanner;
	/**
	 * The JDT Core preferences.
	 * 
	 * @since 3.2
	 ...
	 * @param scanner
	 *            the {@link OvertureHeuristicScanner} to be used for scanning the
	 ...
	 * @since 3.1
	 */
	public OvertureIndenter(IDocument document, OvertureHeuristicScanner scanner,
	...
	private StringBuffer getReferenceIndentation(int offset,
		...
		if (unit == OvertureHeuristicScanner.NOT_FOUND)
			return null;
	...
	public StringBuffer computeIndentation(int offset,
		...
		if (fAlign != OvertureHeuristicScanner.NOT_FOUND) {
			try {
	...
	private StringBuffer getLeadingWhitespace(int offset) {
		...
		try {
			...
			ITypedRegion partition = TextUtilities.getPartition(fDocument,
					IOverturePartitions.OVERTURE_PARTITIONING, lineOffset, true);
			String type = partition.getType();
			while ((type.equals(IOverturePartitions.OVERTURE_DOC) || type
					.equals(IOverturePartitions.OVERTURE_COMMENT))
					&& lineOffset > 0) {
				IRegion tmpLine = fDocument
						.getLineInformationOfOffset(lineOffset - 1);
				partition = TextUtilities.getPartition(fDocument,
						IOverturePartitions.OVERTURE_PARTITIONING, tmpLine
								.getOffset(), false);
	 ...
	 *         should be indented, or {@link OvertureHeuristicScanner#NOT_FOUND}
	 */
		...
		if (offset < fDocument.getLength()) {
			try {
				...
				int prevToken = fScanner.previousToken(prevPos,
						OvertureHeuristicScanner.UNBOUND);
				boolean bracelessBlockStart = fScanner.isBracelessBlockStart(
						prevPos, OvertureHeuristicScanner.UNBOUND);
	 ...
	 * to <code>OvertureHeuristicScanner.NOT_FOUND</code>.
	 * 
	 * @param offset
	 *            the offset for which the reference is computed
	 * @param danglingElse
	...
	public int findReferencePosition(int offset, boolean danglingElse,
		...
		fAlign = OvertureHeuristicScanner.NOT_FOUND;
		fPosition = offset;
		...
		case Symbols.TokenEOF:
			...
			return OvertureHeuristicScanner.NOT_FOUND;
	...
	private boolean skipScope() {
		...
		case Symbols.TokenGREATERTHAN:
			...
			switch (fToken) {
			case Symbols.TokenIDENT:
				if (!OvertureHeuristicScanner.isGenericStarter(getTokenContent()))
					break;
			case Symbols.TokenQUESTIONMARK:
			case Symbols.TokenGREATERTHAN:
	...
	private void nextToken(int start) {
		fToken = fScanner
				.previousToken(start - 1, OvertureHeuristicScanner.UNBOUND);
		fPreviousPos = start;
