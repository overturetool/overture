...
public class VdmBreakpointManager implements IBreakpointListener,
	...
	protected static String makeWatchpointExpression(
			IVdmWatchpoint watchpoint) throws CoreException {
...
//	}
	...
	protected void changeBreakpoint(final IDbgpSession session,
		...
		if (breakpoint instanceof IVdmMethodEntryBreakpoint) {
			...
			if (id != null) {
				...
				if (breakpoint instanceof IVdmWatchpoint) {
					config
							.setExpression(makeWatchpointExpression((IVdmWatchpoint) breakpoint));
				}
	...
	private int classifyBreakpointChange(IMarkerDelta delta,
		...
		final boolean conditional = VdmBreakpointUtils
		...
		final boolean oldExprState = delta.getAttribute(
				AbstractVdmBreakpoint.EXPRESSION_STATE, false);
		final String oldExpr = delta.getAttribute(
				AbstractVdmBreakpoint.EXPRESSION, null);
	...
	public void initializeSession(IDbgpSession session, IProgressMonitor monitor) {
		...
		for (int i = 0; i < breakpoints.length; i++) {
			try {
				...
				VdmDebugPlugin.logWarning(
						NLS.bind("ErrorSetupDeferredBreakpoints", e
								.getMessage()), e);
