...
public class RemoveSourceFolderFromBuildPathCommandHandler extends
		AbstractHandler
{
	...
	public Object execute(ExecutionEvent event) throws ExecutionException
	{
		...
		if (vdmProject != null)
		{
			vdmProject.getModelBuildPath().remove(c);
			try
			{
				...
				Job job = new Job("Rebuild")
				{
					...
					protected IStatus run(IProgressMonitor monitor)
					{
						...
						} catch (CoreException e)
						{
							...
							return new Status(IStatus.ERROR, IVdmUiConstants.PLUGIN_ID, "Faild to rebuild after model path removed", e);
						}
