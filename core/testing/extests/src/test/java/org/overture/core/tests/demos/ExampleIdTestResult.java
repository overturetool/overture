...
/**
 * The {@link ExampleIdTestResult} class holds test result info for
 * an identify test on an Overture example. It is very similar to a counterpart
 * class in the main test demos but since those are not published in maven, they 
 * can never be reused.
 * @author ldc
 *
 */
public class ExampleIdTestResult extends Vector<String> implements List<String>
{
	/**
	 * 
	 */
	 ...
	 *            AST itself.
	 * @return a new instance of {@link ExampleIdTestResult}
	 */
	public ExampleIdTestResult(List<INode> ast)
	{
		super();
		for (INode n : ast)
		{
			if (n instanceof AModuleModules) // ModuleModules prints the file path so we skip it
			{
				...
				{
					super.add(p.toString());
				}

			} else
			{
				super.add(n.toString());
			}
		}
	}
	 ...
	 * <br>
	 * With the {@link ExampleIdTestResult} case we have implemented the comparison ina static method on the
	 * {@link ExampleIdTestResult} class that takes 2 results but it could easily have been done as an instance method that
	 ...
	 *            name of the test
	 */
	public static void compare(ExampleIdTestResult actual, ExampleIdTestResult expected,
			String testName)
	{
