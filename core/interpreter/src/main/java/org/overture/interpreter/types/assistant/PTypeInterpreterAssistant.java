...
public class PTypeInterpreterAssistant
{
	...
	public static boolean isType(PTypeInterpreter b,
			Class<? extends PTypeInterpreter> typeclass)
	{
		return typeclass.isInstance(b);
	}

	public static PTypeInterpreter polymorph(PTypeInterpreter type,
			LexNameToken pname, PTypeInterpreter actualType)
	{
		...
		switch (type.kindPTypeInterpreter())
		{
			case FUNCTION:
				...
				AFunctionTypeInterpreter ftype = new AFunctionTypeInterpreter(location, false, definitions, ((AFunctionTypeInterpreter) type).getPartial(), polyparams, polyresult);
				return ftype;
			case MAP:
				return new AMapMapTypeInterpreter(location, false, definitions, polymorph(((AMapMapTypeInterpreter) type).getFrom(), pname, actualType), polymorph(((AMapMapTypeInterpreter) type).getTo(), pname, actualType), ((AMapMapTypeInterpreter) type).getEmpty());
			case OPTIONAL:
				return new AOptionalTypeInterpreter(location, false, definitions, polymorph(type, pname, actualType));
			case PRODUCT:
			...
			case SEQ:
				return new ASeqSeqTypeInterpreter(location, false, definitions, polymorph(((SSeqTypeInterpreter) type).getSeqof(), pname, actualType), ((SSeqTypeInterpreter) type).getEmpty());
			case SET:
				return new ASetTypeInterpreter(location, false, definitions, polymorph(((ASetTypeInterpreter) type).getSetof(), pname, actualType), ((ASetTypeInterpreter) type).getEmpty(), false);
			case UNION:
	...
	// case PARAMETER:
	// return new AMapMapType(type.getLocation(), false, new AUnknownType(type.getLocation(), false), new
	// AUnknownType(type.getLocation(), false), true);
	// case UNION:
	...
	public static SMapTypeInterpreter getMap(PTypeInterpreter to)
	{
		switch (to.kindPTypeInterpreter())
		{
			case BASIC:
				break;
			case BRACKET:
				return getMap( ((ABracketTypeInterpreter)to).getType());
				break;
			case CLASS:
				break;
			case FUNCTION:
				break;
			case INVARIANT:
				switch(((SInvariantTypeInterpreter) to).kindSInvariantTypeInterpreter())
				{
					case NAMED:
						return getMap(((ANamedInvariantTypeInterpreter)to).getType());
						break;
					case RECORD:
						break;
				...
				break;
			case MAP:
				return (AMapMapTypeInterpreter) to;
				break;
			case OPERATION:
				break;
			case OPTIONAL:
				return getMap(((AOptionalTypeInterpreter)to).getType());
				break;
			case PARAMETER:
				return new AMapMapTypeInterpreter(to.getLocation(),false,null,null,false); // Unknown |-> Unknown
				break;
			case PRODUCT:
				break;
			case QUOTE:
			...
			case UNION:
			{
				AUnknownTypeInterpreter t = (AUnknownTypeInterpreter) to;
				if (!t.getmapDone)
				{
