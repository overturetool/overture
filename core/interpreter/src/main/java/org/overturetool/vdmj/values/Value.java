...
import org.overture.interpreter.ast.types.AParameterTypeInterpreter;
...
abstract public class Value implements Comparable<Value>, Serializable, Formattable, Cloneable
{
	...
	public Value convertTo(PTypeInterpreter to, Context ctxt) throws ValueException
	{
	...
	{
		if (to instanceof AUnionTypeInterpreter)
		{
			AUnionTypeInterpreter uto = (AUnionTypeInterpreter)to;

			for (PTypeInterpreter ut: uto.getTypes())
			{
				try
				{
		...
		else if (to instanceof AParameterTypeInterpreter)
		{
			AParameterTypeInterpreter pt = (AParameterTypeInterpreter)to;
		...
		else if (to instanceof AOptionalTypeInterpreter)
		{
			AOptionalTypeInterpreter ot = (AOptionalTypeInterpreter)to;
			return convertValueTo(ot.getType(), ctxt);
		}
		else if (to instanceof ABracketTypeInterpreter)
		{
			ABracketTypeInterpreter bt = (ABracketTypeInterpreter)to;
			return convertValueTo(bt.getType(), ctxt);
		}
		else if (to instanceof ANamedInvariantTypeInterpreter)
		{
			ANamedInvariantTypeInterpreter ntype = (ANamedInvariantTypeInterpreter)to;
			Value converted = convertValueTo(ntype.getType(), ctxt);
			return new InvariantValue(ntype, converted, ctxt);
		}
		else if (to instanceof AUnknownTypeInterpreter)
		{
