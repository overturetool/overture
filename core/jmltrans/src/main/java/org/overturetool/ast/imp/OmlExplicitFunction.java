...
// ***** VDMTOOLS END Name=setTrailer
  ...
  public String toString() {
	  ...
	  int i, sizeVar = this.ivTypeVariableList.size();
	  String paramList = new String();
	  int j, sizeParam = this.ivParameterList.size();
	  
	  if (sizeVar > 0) {
		  varList += "[";
		  for(i=0; i<sizeVar-1; i++) {
			  varList += this.ivTypeVariableList.get(i).toString() + ", ";
		  }
		  varList += this.ivTypeVariableList.get(sizeVar-1).toString() + "]";
	  }
	  if (sizeParam > 0) {
		  
		  for(j=0; j<sizeParam-1; j++) {
			  paramList += this.ivParameterList.get(j).toString() + ", ";
		  }
		  paramList += this.ivParameterList.get(sizeParam-1).toString();
	  }
	  
	  return this.ivIdentifier + varList + " : "  + this.ivType.toString() + "\n" +
	  this.ivIdentifier + paramList + " == " + this.ivBody.toString() + 
	  this.ivTrailer.toString() + ";\n\n";
  }
}
;
