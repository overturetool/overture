...
public class TypeCheckerStmVisitor extends
	...
	public PType caseAAssignmentStm(AAssignmentStm node, TypeCheckInfo question)
		...
		if (!TypeComparator.compatible(node.getTargetType(), node.getExpType())) {
			...
			TypeCheckerErrors.detail2("Target", node.getTarget(), "Expression",
					node.getExp());
		}
		...
		if (node.getInConstructor()) {
			...
			PDefinition state;
			state = PStateDesignatorAssistantTC.targetDefinition(
					node.getTarget(), question);
	...
	public PType caseAForPatternBindStm(AForPatternBindStm node,
		...
		if (PTypeAssistantTC.isSeq(stype)) {
			...
			TypeCheckerErrors.report(3223,
					"Expecting sequence type after 'in'", node.getLocation(),
					node);
		}
	...
	public PType defaultSSimpleBlockStm(SSimpleBlockStm node,
		...
		for (PStm stmt : node.getStatements()) {
			...
			if (notreached) {
				TypeCheckerErrors.warning(5006,
						"Statement will not be reached", stmt.getLocation(),
						stmt);
			} else {
				last = stype;
				notreached = true;
		...
		if (last != null
				&& (PTypeAssistantTC.isType(last, AVoidType.class) || PTypeAssistantTC
						.isUnknown(last))) {
			rtypes.add(AstFactory.newAVoidType(node.getLocation()));
		}

		node.setType(rtypes.isEmpty() ? AstFactory.newAVoidType(node
				.getLocation()) : rtypes.getType(node.getLocation()));
		return node.getType();
	}
	...
	public PType caseACallObjectStm(ACallObjectStm node, TypeCheckInfo question)
		...
		if (!PTypeAssistantTC.isClass(dtype)) {
			TypeCheckerErrors.report(3207,
					"Object designator is not an object type",
					node.getLocation(), node);
		...
		if (Settings.dialect == Dialect.VDM_RT
			...
			if (!PTypeAssistantTC.isType(atypes.get(0), AClassType.class)) {
				TypeCheckerErrors.report(3280,
						"Argument to deploy must be an object", node.getArgs()
								.get(0).getLocation(), node.getArgs().get(0));
			}
			...
			if (!(atypes.get(0) instanceof AOperationType)) {
				TypeCheckerErrors.report(3290,
						"Argument to setPriority must be an operation", node
								.getArgs().get(0).getLocation(), node.getArgs()
								.get(0));
			} else {
		...
		if (PTypeAssistantTC.isOperation(type)) {
			...
			ACallObjectStatementAssistantTC.checkArgTypes(type,
					optype.getParameters(), atypes); // Not necessary?
			...
			ACallObjectStatementAssistantTC.checkArgTypes(type,
					ftype.getParameters(), atypes); // Not necessary?
			node.setType(ftype.getResult());
			return node.getType();
		} else {
			TypeCheckerErrors.report(3210,
					"Object member is neither a function nor an operation",
					node.getLocation(), node);
			node.setType(AstFactory.newAUnknownType(node.getLocation()));
			return node.getType();
		}
	...
	public PType caseACallStm(ACallStm node, TypeCheckInfo question)
		...
		if (PTypeAssistantTC.isOperation(type)) {
			...
			ACallStmAssistantTC.checkArgTypes(node, optype,
					optype.getParameters(), atypes);
			node.setType(optype.getResult());
			...
			ACallStmAssistantTC.checkArgTypes(node, ftype,
					ftype.getParameters(), atypes);
			node.setType(ftype.getResult());
			return ftype.getResult();
		} else {
			TypeCheckerErrors.report(3210,
					"Name is neither a function nor an operation",
					node.getLocation(), node);
			node.setType(AstFactory.newAUnknownType(node.getLocation()));
			return node.getType();
		}
	...
	public PType caseACaseAlternativeStm(ACaseAlternativeStm node,
		...
		if (node.getDefs().size() == 0) {
			...
			node.getDefs().addAll(
					PPatternAssistantTC.getDefinitions(node.getPattern(), stm
							.getExp().getType(), NameScope.LOCAL));
		}

		PDefinitionListAssistantTC.typeCheck(node.getDefs(), rootVisitor,
				question);
	...
	public PType caseAElseIfStm(AElseIfStm node, TypeCheckInfo question)
		...
		if (!PTypeAssistantTC.isType(
			...
			TypeCheckerErrors.report(3218, "Expression is not boolean",
					node.getLocation(), node);
		}
	...
	public PType caseAForAllStm(AForAllStm node, TypeCheckInfo question)
		...
		if (PTypeAssistantTC.isSet(node.getType())) {
			...
			TypeCheckerErrors.report(3219,
					"For all statement does not contain a set type",
					node.getLocation(), node);
			node.setType(AstFactory.newAUnknownType(node.getLocation()));
			return node.getType();
		}
	...
	public PType caseAForIndexStm(AForIndexStm node, TypeCheckInfo question)
		...
		if (!PTypeAssistantTC.isNumeric(ft)) {
			TypeCheckerErrors.report(3220, "From type is not numeric",
					node.getLocation(), node);
		}

		if (!PTypeAssistantTC.isNumeric(tt)) {
			TypeCheckerErrors.report(3221, "To type is not numeric",
					node.getLocation(), node);
		}
		...
		if (node.getBy() != null) {
			...
			if (!PTypeAssistantTC.isNumeric(bt)) {
				TypeCheckerErrors.report(3222, "By type is not numeric",
						node.getLocation(), node);
			}
		}
	...
	public PType caseAIfStm(AIfStm node, TypeCheckInfo question)
		...
		if (!PTypeAssistantTC.isType(test, ABooleanBasicType.class)) {
			TypeCheckerErrors.report(3224, "If expression is not boolean", node
					.getIfExp().getLocation(), node.getIfExp());
		}
	...
	public PType caseANonDeterministicSimpleBlockStm(
		...
		for (PStm stmt : node.getStatements()) {
			...
			if (PTypeAssistantTC.isType(stype, AUnionType.class)) {
				AUnionType ust = (AUnionType) stype;
				for (PType t : ust.getTypes()) {
					if (ANonDeterministicSimpleBlockStmAssistantTC.addOne(
							rtypes, t))
						rcount++;
				}
			} else {
				if (ANonDeterministicSimpleBlockStmAssistantTC.addOne(rtypes,
						stype))
					rcount++;
			}
		}
		...
		if (rcount > 1) {
			TypeCheckerErrors.warning(5016,
					"Some statements will not be reached", node.getLocation(),
					node);
		}
	...
	public PType caseASpecificationStm(ASpecificationStm node,
		...
		if (node.getErrors() != null) {
			for (AErrorCase err : node.getErrors()) {
				...
				if (!PTypeAssistantTC.isType(lt, ABooleanBasicType.class)) {
					TypeCheckerErrors.report(3275,
							"Error clause must be a boolean", err.getLeft()
									.getLocation(), err.getLeft());
				}

				if (!PTypeAssistantTC.isType(rt, ABooleanBasicType.class)) {
					TypeCheckerErrors.report(3275,
							"Error clause must be a boolean", err.getRight()
									.getLocation(), err.getRight());
				}
			}
		}
		...
		if (node.getPrecondition() != null
			...
			TypeCheckerErrors.report(3233,
					"Precondition is not a boolean expression", node
							.getPrecondition().getLocation(), node
							.getPrecondition());
		}
		...
		if (node.getPostcondition() != null
			...
			TypeCheckerErrors.report(3234,
					"Postcondition is not a boolean expression", node
							.getPostcondition().getLocation(), node
							.getPostcondition());
		}
	...
	public PType caseATrapStm(ATrapStm node, TypeCheckInfo question)
		...
		if (extype.isEmpty()) {
			TypeCheckerErrors.report(3241,
					"Body of trap statement does not throw exceptions",
					node.getLocation(), node);
	...
	public PType caseAPeriodicStm(APeriodicStm node, TypeCheckInfo question) 
		...
		if (opdef == null) {
			TypeCheckerErrors.report(3228, opname + " is not in scope",
					node.getLocation(), node);
			node.setType(AstFactory.newAUnknownType(node.getLocation()));
			return node.getType();
		}
		...
		AOperationType expected = AstFactory.newAOperationType(
				node.getLocation(), new Vector<PType>(),
				AstFactory.newAVoidType(node.getLocation()));
		...
		if (opdef instanceof AExplicitOperationDefinition) {
			...
			if (!PTypeAssistantTC.equals(def.getType(), expected)) {
				TypeCheckerErrors.report(3229, opname
						+ " should have no parameters or return type",
						node.getLocation(), node);
			...
			if (def.getBody() == null) {
				TypeCheckerErrors.report(3230, opname + " is implicit",
						node.getLocation(), node);
			}
			...
			TypeCheckerErrors.report(3232,
					opname + " is not an operation name", node.getLocation(),
					node);
		}
	...
	public PType caseAStartStm(AStartStm node, TypeCheckInfo question)
		...
		if (PTypeAssistantTC.isSet(type)) {
			...
			if (!PTypeAssistantTC.isClass(set.getSetof())) {
				TypeCheckerErrors.report(3235,
						"Expression is not a set of object references", node
								.getObj().getLocation(), node.getObj());
			} else {
				AClassType ctype = PTypeAssistantTC
						.getClassType(set.getSetof());

				if (SClassDefinitionAssistantTC.findThread(ctype.getClassdef()) == null) {
					TypeCheckerErrors.report(3236,
							"Class does not define a thread", node.getObj()
									.getLocation(), node.getObj());
				}
			}
			...
			TypeCheckerErrors
					.report(3238,
							"Expression is not an object reference or set of object references",
	...
	public PType caseADefPatternBind(ADefPatternBind node,
		...
		if (bind != null) {
			if (bind instanceof ATypeBind) {
				...
				ATypeBindAssistantTC.typeResolve(typebind, rootVisitor,
						question);

				if (!TypeComparator.compatible(typebind.getType(), type)) {
					TypeCheckerErrors.report(3198,
							"Type bind not compatible with expression",
							bind.getLocation(), bind);
				...
				if (!TypeComparator.compatible(type, settype.getSetof())) {
					TypeCheckerErrors.report(3199,
							"Set bind not compatible with expression",
							bind.getLocation(), bind);
			...
			PDefinition def = AstFactory.newAMultiBindListDefinition(
					bind.getLocation(),
					PBindAssistantTC.getMultipleBindList(bind));
