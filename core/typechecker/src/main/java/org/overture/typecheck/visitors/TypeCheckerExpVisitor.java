	...
	public PType caseACompBinaryExp(ACompBinaryExp node, TypeCheckInfo question) {
		...
		if (PTypeAssistant.isMap(node.getLeft().getType()))
		{
    		if (!PTypeAssistant.isMap(node.getRight().getType()))
    		{
    			...
    			node.setType(new AMapMapType(node.getLocation(),false,null, new AUnknownType(node.getLocation(), false),  new AUnknownType(node.getLocation(), false),true));	// Unknown types
    			return node.getType(); 
    		}
...
//    					impdef.getType(actualTypes) : expdef.getType(actualTypes);
	...
	public PType caseAMapCompMapExp(AMapCompMapExp node, TypeCheckInfo question) {
		...
		if (predicate != null && !PTypeAssistant.isType(predicate.apply(rootVisitor, new TypeCheckInfo(local,question.scope,question.qualifiers)), ABooleanBasicType.class))
		{
			TypeCheckerErrors.report(3118, "Predicate is not boolean",predicate.getLocation(),predicate);
		}

		
		node.setType(node.getFirst().apply(rootVisitor, new TypeCheckInfo(local,question.scope,question.qualifiers)));	// The map from/to type
		local.unusedCheck();
		return node.getType();
	}
	...
	public PType caseADistMergeUnaryExp(ADistMergeUnaryExp node,
		...
		return new AMapMapType(node.getLocation(),false, new AUnknownType(node.getLocation(), false), new AUnknownType(node.getLocation(), false),true);	// Unknown types
	}
	...
	public PType caseAElementsUnaryExp(AElementsUnaryExp node,
		...
		node.setType(seq.getEmpty()? new ASetType(node.getLocation(),false,new AUnknownType(node.getLocation(), false),true,false) : new ASetType(node.getLocation(),false, seq.getSeqof().clone(),false,false));
		return node.getType();
	}
}
