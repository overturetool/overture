		...
		extends QuestionAnswerAdaptor<POContextStack, ProofObligationList>
{
			...
			POContextStack question) throws AnalysisException
	{
		...
		if (!question.assistantFactory.createPTypeAssistant().isUnknown(type)
				&& question.assistantFactory.createPTypeAssistant().isFunction(type))
		{
			AFunctionType funcType = question.assistantFactory.createPTypeAssistant().getFunction(type);
			ILexNameToken prename = assistantFactory.createPExpAssistant().getPreName(root);
			if (prename == null
					|| !prename.equals(PExpAssistantTC.NO_PRECONDITION))
			{
				obligations.add(new FunctionApplyObligation(node.getRoot(), node.getArgs(), (prename == null ? null
						: prename.getName()), question));
			}
			int i = 0;
			...
			POContextStack question) throws AnalysisException
	{
		...
		if (!question.assistantFactory.createPTypeAssistant().isType(exp.getType(), ASeq1SeqType.class))
			...
			POContextStack question) throws AnalysisException
	{
		...
		for (ACaseAlternative alt : node.getCases())
		{
			...
			obligations.addAll(question.assistantFactory.createACaseAlternativeAssistant().getProofObligations(alt, rootVisitor, question, node.getExpression().getType()));
			/*
	...
	public ProofObligationList caseADefExp(ADefExp node, POContextStack question)
			throws AnalysisException
	{
		ProofObligationList obligations = question.assistantFactory.createPDefinitionAssistant().getProofObligations(node.getLocalDefs(), rootVisitor, question);
	...
	@Override
	public ProofObligationList caseANarrowExp(ANarrowExp node,
			POContextStack question) throws AnalysisException
	{
		ProofObligationList obligations = new ProofObligationList();

		PType expected = (node.getTypedef() == null ? node.getBasicType()
				: question.assistantFactory.createPDefinitionAssistant().getType(node.getTypedef()));
			...
			throws AnalysisException
	{
		...
		{
			obligations.add(new org.overture.pog.obligation.InvariantObligation(node, question));
		}
		return obligations;
	}
			...
			POContextStack question) throws AnalysisException
	{
		...
		if (!question.assistantFactory.createPTypeAssistant().isType(node.getExp().getType(), ASeq1SeqType.class))
			...
			POContextStack question) throws AnalysisException
	{
		...
		{
			obligations.add(new org.overture.pog.obligation.SeqModificationObligation(node, question));
		}
			...
			POContextStack question)
	{
		...
		if (question.assistantFactory.createPTypeAssistant().isFunction(lType))
		{
			...
			if (preName == null
					|| !preName.equals(PExpAssistantTC.NO_PRECONDITION))
			{
				obligations.add(new org.overture.pog.obligation.FuncIterationObligation(node, preName.getName(), question));
			}
		}
