...
import org.overture.pog.POScopeContext;
import org.overture.pog.ProofObligationList;
import org.overture.pog.StateInvariantObligation;
import org.overture.pog.SubTypeObligation;
import org.overture.pog.WhileLoopObligation;
...
public class PogParamStmVisitor<Q extends POContextStack, A extends ProofObligationList>
	...
	public ProofObligationList caseAAlwaysStm(AAlwaysStm node,
		...
		try {
			ProofObligationList obligations = node.getAlways().apply(
	...
	public ProofObligationList caseAElseIfStm(AElseIfStm node,
			POContextStack question) throws AnalysisException {
		try {
			ProofObligationList obligations = node.getElseIf().apply(
	...
	public ProofObligationList caseAForAllStm(AForAllStm node,
			POContextStack question) throws AnalysisException {
		try {
			ProofObligationList obligations = node.getSet().apply(rootVisitor,
					question);
			obligations
	...
	public ProofObligationList caseAForIndexStm(AForIndexStm node,
			POContextStack question) throws AnalysisException {
		try {
			ProofObligationList obligations = node.getFrom().apply(rootVisitor,
					question);
	...
	public ProofObligationList caseAForPatternBindStm(AForPatternBindStm node,
			POContextStack question) throws AnalysisException {
		try {
			ProofObligationList list = node.getExp().apply(rootVisitor,
					question);
	...
	public ProofObligationList caseAIfStm(AIfStm node, POContextStack question)
			throws AnalysisException {
		try {
			ProofObligationList obligations = node.getIfExp().apply(
					rootVisitor, question);
			obligations.addAll(node.getThenStm().apply(mainVisitor, question));
