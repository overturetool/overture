...
public class PogTestHelper
{
			...
			List<String> expected)
	{
		...
		StringComparison closestCompSoFar = new StringComparison(actual, expected.get(0), editDistance(actual, expected.get(0)));

		for (int i = 1; i < expected.size(); i++)
		{
			StringComparison currentComp = new StringComparison(actual, expected.get(i), editDistance(actual, expected.get(i)));
	...
	{
		return comp.getDistLengthRatio() > 0
				&& comp.getDistLengthRatio() < CHECK_TOLERANCE;
	}
