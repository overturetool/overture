...
public class VdmToBConverter extends DepthFirstAnalysisAdaptorAnswer<Node>
{
	 ...
	 *           AMemberPredicate(exp(node.getSetBind().getPattern()), exp(node.getSetBind().getSet())));
	 *           scs.setPredicates(new AConjunctPredicate(scs.getPredicates(), pred(node.getPredicate())));
	 *           List<TIdentifierLiteral> ident = new ArrayList<TIdentifierLiteral>(); ident.add(new
	 *           TIdentifierLiteral("_target_")); PExpression temp = new AIdentifierExpression(ident);
	 *           scs.getIdentifiers().add(temp); scs.setPredicates(new AConjunctPredicate(scs.getPredicates(), new
	 *           AEqualPredicate(temp, exp(node.getFirst())))); if(scs.getMembers().isEmpty()) { AEmptySequeneExpression
	 *           seq = new AEmptySequeneExpression(); } else { ASequenceExtensionExpression seq = new
	 *           ASequenceExtensionExpression(); for(PExpression elem : scs.getMembers()) {
	...
	public Node caseATupleExp(ATupleExp node) throws AnalysisException
	{
		...
		/*
		 * cpl.getList().add(exp(args.get(0))); cpl.getList().add(exp(args.get(1)));
		 */
