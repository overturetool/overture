...
public class StoreAssistant
{
	...
	public StoreAssistant(TraceNames tracePrefixes, TransAssistantIR transAssistant)
	{
		super();
	...
	public void appendStoreRegStms(ABlockStmIR declBlock, String varName, String idConstName, boolean staticReg)
	{
	...
	public AVarDeclIR consIdConstDecl(String idConstName)
	{
		...
		SExpIR init = transAssistant.consInstanceCall(idGenType, tracePrefixes.idGeneratorVarName(),
	...
	public ACallObjectExpStmIR consStoreRegistration(String idConstName, STypeIR varType, String varName, boolean staticReg)
	{
	...
	public ACallObjectExpStmIR consStoreRegistration(String idConstName, SExpIR subject, boolean staticReg)
	{
	...
	public SExpIR consStoreLookup(AIdentifierVarExpIR node, boolean noCast)
	{
		...
		AIdentifierVarExpIR idArg = transAssistant.getInfo().getExpAssistant().consIdVar(idConstNameMap.get(node.getName()), 
				new ANatNumericBasicTypeIR());
		
		SExpIR call = transAssistant.consInstanceCall(storeType, tracePrefixes.storeVarName(), 
				node.getType(), tracePrefixes.storeGetValueMethodName(), idArg);
		
		if (noCast || node.getType() instanceof AUnionTypeIR || node.getType() instanceof AUnknownTypeIR
