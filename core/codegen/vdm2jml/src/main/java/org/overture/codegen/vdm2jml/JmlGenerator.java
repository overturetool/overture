...
public class JmlGenerator implements IREventObserver, IJavaQuoteEventObserver
{
	...
	private void tcTraceTest(TypePredDecorator assertTr)
	{
		for (TcExpInfo currentInfo : tcExpInfo)
		{
			...
			currentInfo.getTypeCheck().setMetaData(annotator.consMetaData("//@ "
					+ JML_ASSERT_ANNOTATION + " " + checkStr + ";"));
		}
	}
	...
	private void computeNamedTypeInvInfo(List<AModuleModules> ast)
			throws AnalysisException
	{
	...
	private void addAssertions(List<IRStatus<PIR>> newAst,
			TypePredDecorator assertTr)
	{
		for (IRStatus<ADefaultClassDeclIR> status : IRStatus.extract(newAst, ADefaultClassDeclIR.class))
		{
			ADefaultClassDeclIR clazz = status.getIrNode();
	 ...
	 * calling this method a call to the invariant method from the invariant will take the following form: //@ public
	 * instance invariant inv_Rec(field1,field2);
	 * 
	 * @param rec
	 *            The record for which we will change the invariant method
	 */
	...
	public void adjustNamedTypeInvFuncs(IRStatus<ADefaultClassDeclIR> status)
	{
		...
		for (ATypeDeclIR typeDecl : clazz.getTypeDecls())
		{
			if (typeDecl.getDecl() instanceof ANamedTypeDeclIR)
			{
				...
				method.setName(JmlGenerator.INV_PREFIX + defModule + "_"
						+ name);
