...
public class RecModUtil
{
	...
	public boolean simpleRecSetCallOutsideAtomic(ACallObjectExpStmIR node)
	{
		...
		if (obj instanceof ACastUnaryExpIR
				&& ((ACastUnaryExpIR) obj).getExp() instanceof SVarExpIR)
		{
			return true;
		}
	...
	public AMetaStmIR handleRecAssert(SExpIR var, String varName,
			ARecordTypeIR recType)
	{
		return handler.getInvTrans().consMetaStm(consValidRecCheck(var, varName, recType));
	}

	public String consValidRecCheck(SExpIR subject, String varName,
			ARecordTypeIR recType)
	{
	...
	{
		if (exp.getType().getNamedInvType() != null
				|| !(exp.getType() instanceof ARecordTypeIR))
		{
			return false;
		}
