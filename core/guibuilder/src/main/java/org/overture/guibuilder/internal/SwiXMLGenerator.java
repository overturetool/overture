...
public class SwiXMLGenerator
{
	 ...
	 * 
	 * @param classReader
	 *            A vdm class reader to extract required information
	 * @param title
	 *            The title name of the main window
	 */
	 ...
	 * 
	 * @param vector
	 *            List of vdm classes that ui fronts for
	 * @param title
	 *            Title of teh main window
	...
	private Frame createMainWindow(Vector<IVdmDefinition> vector, String title)
	{
		...
		for (IVdmDefinition c : vector)
		{
			...
			/*
			 * if ( c.getName().equals("Till") ) button.setEnabled(false);
			 */
	 ...
	 * 
	 * @param classDef
	 *            The vdm class of the instances
	 */
	...
	/**
	 * Defines the panel that contains elements to interact with a single method a vdm class
	 * 
	 * @param className
	 *            The vdm class
	 * @param def
	 *            The method of the vdm class
	 * @return
	 */
	 ...
	 * 
	 * @param panel
	 *            The panel into which add the elements
	 * @param def
	 *            The vdm method the ui elements are intended for.
	 */
	...
	private void addGenericElements(Panel panel, VdmMethod def)
	{
		...
		for (int i = 0; i < def.getParamList().size(); ++i)
		{
			...
			// the input widget
			Object inputWidget = toUserInterfaceElement(def.getParamList().get(i), NamingPolicies.getInputComponentId(def.getName(), i));
			...
			if (vdmDef instanceof VdmMethod)
			{
				if (((VdmMethod) vdmDef).isConstructor())
				{
					...
					for (int i = 0; i < def.getParamList().size(); ++i)
					{
						...
						methodlbl.setText(def.getParamList().get(i).getParaName());
						panel.getContent().add(methodlbl);
						Object inputWidget = toUserInterfaceElement(def.getParamList().get(i), NamingPolicies.getConstructorWidgetId(classDef.getName(), 0, i));
						panel.getContent().add(inputWidget);
					} // for
					return panel;
				} // isConstructor()
	 ...
	 *            The vdm parameter
	 * @param id
	 *            The id to give to the ui element
