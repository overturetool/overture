...
Models in VDM are formal in the sense that they have a precisely
...
the tools is managed using GitHub.  The best way to run Overture
...
Before the debugging can be initiated in Overture%
%\footnote{In the
...
\item[Remote Console:] This is an advanced option that enables remote
  control of the interpreter and this is described in the Overture
  user manual~\cite{Larsen&13a}.
...
configuration which requires the project
...
options here. Two of these are not explained further here
but the one called \texttt{Generate
 Coverage} is important for you to check in case you would like
...
traces can be written directly as a part of a VDM-SL model. Here the
...
\begin{lstlisting}
java -jar Overture-2.0.0.jar -vdmsl AlarmSL
\end{lstlisting}
...
java -jar Overture-2.0.0.jar -vdmsl -q -e 
...
type check, this will cause Overture to enter its interactive mode, and
...
commands is described in the Overture User Guide\footnote{Supplied with
